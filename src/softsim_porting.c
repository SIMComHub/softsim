...
SOFTSIM_bool system_set_apn(void)
{
  ...
  if (qapi_DAM_Visual_AT_Open(NULL))
  {
      ...
      for (i = 0; i < 3; ++i)
      {
          ...
          if (strstr((const char *)apn_buffer, "OK"))
          {
              QAPI_MSG_SPRINTF(MSG_SSID_LINUX_DATA, MSG_LEGACY_HIGH, "at+CGAUTH OK, ret=%d", ret);
              break;
          }
          else
          {
              QAPI_MSG_SPRINTF(MSG_SSID_LINUX_DATA, MSG_LEGACY_HIGH, "at+CGAUTH ERROR, ret=%d,%s", ret, apn_buffer);
          }
      }
  }
  else
	        ...
	        else if ((QUEC_AT_MASK_NA_V01 | QUEC_AT_MASK_EQ_V01 | QUEC_AT_MASK_AR_V01) == mask)//AT+PSOFTSIM=<value>
	        {
                ...
                else if (!strncmp((char *)at_fwd_params, "start", 5))
                {
                    ...
                    if (at_fwd_params[5] == 0x00)
                    {
                        ...
                        {
                            sprintf((char *)p_out_buffer, "210301 apn_settled %d auth_settled %d entry_delay:%d", apn_settled, auth_settled, SOFTSIM_ENTRY_DELAY);
